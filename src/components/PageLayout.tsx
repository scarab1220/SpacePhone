  ...
  return (
    <section className="relative pt-32 pb-12 md:pt-40 md:pb-20 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        {eyebrow && (
          <div className="flex items-center gap-2 mb-3">
            {icon === "apple" ? (
              <svg
                viewBox="0 0 384 512"
                className="w-5 h-5 fill-current text-primary-bright"
                aria-hidden="true"
              >
        ...
        <h1 className="text-4xl md:text-6xl font-bold text-on-surface tracking-tight mb-4">
          {title}
        </h1>
        {description && (
...
export function Breadcrumbs({
  ...
  trail: { label: string; to?: string; params?: Record<string, string> }[];
}) {
  return (
...
export function ProductGrid({
  products,
  emptyMessage = "Aún no hay productos en esta sección.",
}: {
  products: Product[];
  emptyMessage?: string;
}) {
